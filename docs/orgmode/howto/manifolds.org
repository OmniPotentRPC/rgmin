...
| ~CenteredMatrix { m, n, rows }~ | row-major ~m x n~, length ~m n~ | ~X + U~ then center |
...
rgmin_solver_set_centered_matrix(s, 2, 3, 0);
rgmin_solver_set_manifold(s, RGMIN_MANIFOLD_CENTERED_MATRIX);
...
- ~CenteredMatrix { m, n, rows }~ is manopt
  ...
  row-major (~m n~). Default ~rows = false~ (~'cols'~) subtracts
  the mean column so ~X 1_n = 0~. ~rows = true~ (~'rows'~)
  subtracts the mean row so ~1_m^T X = 0~. Projection is that
  centering. Retraction is ~X + U~ then center. Transport is
  the identity. It is not the sphere and not a 3N cluster.
  C token 21 (~rgmin_solver_set_centered_matrix~). Tokens 7-10
  stay reserved.
