...
L\ :sub:`inf`\ trust from ``rgmin_solver_set_highs_trust`` or atom
maxmove, and optional equalities from ``rgmin_solver_add_equality``.
That is the path eOn / rgpot / any eindir objective share. Without the
``highs`` cargo feature every setter returns 1.
